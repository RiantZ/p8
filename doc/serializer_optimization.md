...
Реестр TLS writer'ов нужен **трём** независимым потребителям, у каждого
из которых свой контракт на синхронизацию:

| Потребитель | Зачем |
|---|---|
| Pull-flush (consumer thread, normal operation) | Bounded-latency drain idle/partially-filled writer'ов на расписании |
| Graceful shutdown (`p8_release`) | Вытащить хвостовые данные после остановки producer'ов |
| Exceptional flush (`p8_exceptional_flush`, см. 7.6) | Best-effort dump in-flight состояния из crash-handler'а |

Регистрация **не нужна** для нормального push-пути (producer сам пушит
заполненные `(data, mini)`-пары в `mo_ready_queue` через `submit_chain`).

#### 4.7.1 Принципы синхронизации

##### 4.7.1.1 Владение lock'ами

В системе используются **два разных типа** spin-lock'ов, у каждого свой
владелец и своя зона ответственности:

| Lock | Владелец | Защищает |
|---|---|---|
| `cp8_log::mo_lock` (и аналоги в `cp8_mtk`/`cp8_trc`) | Сам подкласс writer'а | Поля writer'а: `mp_buffer`, `mz_buf_used`, `mp_mini_buffer`, `mz_mini_used`, `mo_fragments`, `mo_mini_fragments`, dedup-state, и TLS-кэши подкласса (`mo_desc_map` и т.п.) |
| `cp8_core::mo_writers_lock` | `cp8_core` | Топологию intrusive-списка реестра: `mp_writers_head` + `mp_next_writer`/`mp_prev_writer` поля в каждом writer'е |

Это **два независимых** lock'а с разными scope'ами. У базового
`cp8_tls_writer` собственного lock'а **нет** — он наследует lock от
своего подкласса.

##### 4.7.1.2 Кто что берёт при доступе к данным writer'а извне

Когда pull-нить ядра приходит к writer'у нити X, чтобы прочитать или
сдвинуть его in-flight состояние, она берёт **тот же** `mo_lock`, что
держит и producer этой же нити на hot-path'е. Других вариантов нет:

- «Глобальный read-lock в core» сделал бы producer'ы зависимыми от
  одной точки синхронизации для **всех** нитей процесса → пробивает
  весь hot-path-бюджет на ровном месте.
- RW-locks на write-heavy нагрузке (10⁸ writes/sec, редкие reads)
  деградируют к обычному mutex'у, никаких преимуществ нет.
- Отдельный «read-only-lock» внутри writer'а пришлось бы брать **и**
  producer'у на каждом emit'е (чтобы не race с reader'ом), что
  удваивает hot-path-overhead без выигрыша.

То есть архитектурно у нас **ровно один** lock на чтение и запись
содержимого writer'а — `mo_lock` подкласса. Реестр (next/prev) защищён
отдельно `mo_writers_lock`'ом, потому что это **другая** структура
данных с другим pattern'ом доступа.

##### 4.7.1.3 «Согласованность» между типами writer'ов — намеренно слабая

У одной нити **три** независимых writer'а (`cp8_log`, `cp8_mtk`, `cp8_trc`),
каждый со своим `mo_lock`. Атомарного snapshot'а «состояние всех трёх в
один момент» не существует — pull-нить берёт lock'и по очереди, и между
ними нить X успевает работать дальше. Это **OK**: writer'ы независимо
пушат свои буферы в общий `mo_ready_queue`; sink получает три потока,
упорядоченных по timestamp'ам, и собирает их обратно на стороне consumer'а
данных (вне рамок этого документа).

##### 4.7.1.4 Hot-path-контракт

Так как **тот же** `mo_lock` берётся и producer'ом на hot-path'е, и
pull-нитью при чтении извне, прямой блокирующий `lock()` из ядра
деградирует producer'а — на следующем emit'е он попадает в busy-spin
`kit::c_spin_lock`'а и теряет 500 ns – 1 μs (типичное время удержания
pull-нитью). Это пробивает p99.9-латентность на high-load — см.
risk-таблицу в разделе 10.

Поэтому контракт acquire'ов разный для разных потребителей (4.7.1.5):
pull-flush делает `try_lock` без ожидания, graceful shutdown берёт
блокирующий `lock()` уже после остановки producer'ов, crash-flush
не берёт lock вообще.

##### 4.7.1.5 Контракт acquire'ов по потребителям

Поля `cp8_tls_writer` — private для своей нити. Внешний потребитель
пишет/читает их **только** удерживая `mo_lock` подкласса (или вообще
не удерживая lock — в случае lock-free crash-flush'а с accepted-risk
partial state):

Контракт acquire'ов по потребителям:

| Потребитель | `mo_writers_lock` | `writer->mo_lock` |
|---|---|---|
| `register_writer` (constructor) | `lock()` | — |
| `unregister_writer` (destructor) | `lock()` | — |
| Pull-flush iterate (normal) | `lock()` на iterate | **`try_lock()`**; на failure — **skip writer**, ловим на следующем проходе |
| Graceful shutdown drain | `lock()` на iterate | `lock()` блокирующий (producer'ы уже остановлены) |
| Exceptional flush (crash) | **lock-free** read `mp_writers_head` | **never lock**, lock-free direct read (7.6) |

`try_lock`-on-skip — ключевое решение, изолирующее hot-path producer'а от
pull-flush'а: producer **никогда не ждёт** ядро. Если pull-нить пришла к
writer'у X в момент, когда producer держит `mo_lock` — pull-нить просто
идёт дальше. На следующем проходе try_lock либо пройдёт (writer вышел из
send), либо снова промахнётся. В обоих случаях данные writer'а X доедут
до sink'а: либо через push-rotation на полном буфере (как обычно), либо
через успешный try_lock'нутый pull-проход. Idle-writer (нить не зашла в
...
`mp_prev_writer` живут **в самом** `cp8_tls_writer` — это позволяет
unregister O(1) без поиска, без аллокации pool-узлов, без зависимости
от внутреннего представления `kit::c_lst`:
...
void cp8_core::register_writer(cp8_tls_writer *ip_w)
{
    ...
    ip_w->mp_prev_writer = nullptr;
    ip_w->mp_next_writer = mp_writers_head;
    if(mp_writers_head)
    {
        mp_writers_head->mp_prev_writer = ip_w;
    }
    mp_writers_head = ip_w;
}

void cp8_core::unregister_writer(cp8_tls_writer *ip_w)
{
    std::lock_guard<kit::c_spin_lock> lo_guard(mo_writers_lock);
    if(ip_w->mp_prev_writer)
    {
        ip_w->mp_prev_writer->mp_next_writer = ip_w->mp_next_writer;
    }
    else
    {
        mp_writers_head = ip_w->mp_next_writer;
    }
    if(ip_w->mp_next_writer)
    {
        ip_w->mp_next_writer->mp_prev_writer = ip_w->mp_prev_writer;
    }
    ip_w->mp_next_writer = nullptr;
    ip_w->mp_prev_writer = nullptr;
}
```
...
Producer hot-path (`cp8_log::send` и аналоги):
- **+0 lock'ов**. `mo_writers_lock` producer вообще не трогает. Свой
  `mo_lock` — без изменений по сравнению с текущей реализацией.
- **+16 bytes** на writer (`mp_next_writer` + `mp_prev_writer`),
  размещаются в той же cache line, что и существующие TLS-поля → нет
  дополнительных промахов.

Lifecycle нити (3 TLS writer'а — `cp8_log`, `cp8_mtk`, `cp8_trc`):
- **6 acquire/release `mo_writers_lock`** на жизнь нити (3 × register +
  3 × unregister). На low/med-churn (десятки–сотни нитей, живут секунды
  и больше) — невидимо в профиле.
- На high-churn (тысячи короткоживущих нитей/сек) `mo_writers_lock`
  становится точкой контеншена → см. open question в разделе 10 про
  sharded registry.

Pull-flush (regular):
- 1 acquire `mo_writers_lock` на iterate.
- N `try_lock` на каждый writer (без ожидания). При промахе — переход
  к следующему writer'у без накладных расходов.
- Hot-path producer'ов **не задет**.

Graceful shutdown:
- 1 acquire `mo_writers_lock` + блокирующий `lock()` каждого writer'а.
- Tail-latency не важна — producer'ы уже остановлены.

Crash-flush:
- Lock-free direct read `mp_writers_head` и обход через
  `mp_next_writer` (7.6). Partial-state допустим.

#### 4.7.4 Accepted-risk: idle writer и pull-flush

Если writer не вызывает `send` (нить спит на IO, ждёт shutdown'а),
pull-flush через try_lock пройдёт без contention'а и спокойно ротирует
буфер. То есть для **полностью idle** нити pull-flush работает.

Проблема возникает только в редком race-сценарии: pull-нить пришла к
writer'у X **ровно** в момент, когда X внутри `send()`. try_lock
промахнётся, pull-нить пропустит этого writer'а. Данные **не теряются**:

- Либо X быстро выйдет из send и его буфер ротируется через обычный
  push-путь.
- Либо pull-нить поймает X на следующем проходе (через N ms).
- Либо в худшем случае данные доедут на graceful shutdown'е (где
  блокирующий `lock()` гарантирует drain).

Этот трейдоф (худшая bounded-latency для пишущего writer'а в обмен на
нулевой overhead на hot-path producer'а) принят сознательно.
...
3. **Partial / corrupt data допустима** — если crash случился посреди
   сериализации log_item'а, прочитанный prefix может быть невалиден
...
void cp8_core::exceptional_flush_signal_safe()
{
    ...
    //    напрямую без acquire mo_lock и mo_writers_lock
    for(cp8_tls_writer *lp_w = mp_writers_head;
        lp_w != nullptr;
        lp_w = lp_w->mp_next_writer)
    {
...
10. **Reset-семантика для режима A**: `on_reset` hooks, `s_p8_sink_known`,
    `prime_known`.
11. **TLS writer registry (раздел 4.7)**: intrusive doubly-linked
    list через поля `mp_next_writer`/`mp_prev_writer` в самом
    `cp8_tls_writer`; голова `mp_writers_head` и spin-lock
    `mo_writers_lock` в `cp8_core`; методы
    `register_writer` / `unregister_writer` (вызываются из конструктора/
    деструктора `cp8_tls_writer`); `peek_first_unlocked` helper в kit
    (или внутренний эквивалент в p8) для lock-free iterate. Регрессионные
    тесты на: register/unregister в нескольких нитях одновременно;
    iterate под `mo_writers_lock` корректно видит снимок реестра;
    unregister O(1) без поиска.
12. **Exceptional flush (раздел 7.6)**: lock-free iterate реестра из
    шага 11 (без acquire `mo_writers_lock` и без acquire `mo_lock`
    каждого writer'а); `write_signal_safe` / `flush_internal_signal_safe`
    в `cp8_sink_iface` и реализации в `cp8_sink_file`. Регрессионные
    тесты: (a) sink получает все ready-queue буферы; (b) sink получает
    in-flight TLS state нескольких живых writer'ов; (c) сценарий "одна
    нить в псевдо-crash'е" (lock держит другая инструментированная
    нить) — crash-flush не deadlock'ается, скипает crashed writer и
    продолжает с другими (через test-helper).
13. **Pull-flush в consumer thread (раздел 4.7)**: extension consumer'а из
    шага 8 — периодический обход `mp_writers_head` под `mo_writers_lock`
    с `try_lock` на `mo_lock` каждого writer'а; на try_lock'е получили —
    финализируем header'ы текущей пары `(data, mini)`, push в
    `mo_ready_queue` через `submit_chain`, сбрасываем поля writer'а;
    на try_lock'е не получили — skip без ожидания. Период pull-flush'а
    конфигурируется через `s_p8_config`. Регрессионные тесты:
    (a) idle writer drain'ится за один pull-период; (b) активно пишущий
    writer не задерживается на hot-path'е — replay-тест p99 emit-латентности
    под одновременным pull-flush'ем; (c) try_lock-промах не вызывает
    потери данных (rotation-on-full всё равно срабатывает).
14. **Graceful shutdown drain в `p8_release`**: после остановки producer'ов
    обход реестра с блокирующим `lock()` каждого writer'а; финальный
    push всех хвостовых буферов; pull-flush'у в consumer thread даётся
    время дренировать ready-queue до закрытия sink'а.
15. **`cp8_sink_net`** — отдельная задача, не входит в этот план; включает
    разработку handshake'а и стратегии переподключения.
16. **Остальные типы service-data**: thread/trace/mtk/module descriptors —
...
- **Pull-flush schedule** (раздел 4.7): частота regular pull-flush'а
  ...
  сверху диктуется суммарным размером TLS-буферов всех writer'ов; снизу —
  стоимостью iterate'а реестра.
...
| **Pull-flush блокирует hot-path producer'а** | Если pull-нить ядра делает блокирующий `lock()` на `writer->mo_lock`, producer на следующем `send()` попадает в busy-spin'е `kit::c_spin_lock`'а и теряет ~500 ns – 1 μs (типичное время удержания pull-нитью под lock'ом). На 10⁸ events/sec p99.9 латентность взлетает с ~30 ns до ~1 μs. Дополнительно: cache-line bouncing `mo_lock`'а между ядром pull-нити и ядром producer'а даёт постоянный 5–10% throughput-deg. | **Решено** (4.7): pull-flush делает `try_lock` на `mo_lock` каждого writer'а; на failure — skip без ожидания, ловим на следующем проходе. Hot-path producer'ов не задет. Блокирующий `lock()` используется только в graceful shutdown, где producer'ы уже остановлены. |
| **Idle-writer не drain'ится pull-flush'ем** | Writer, в котором нить не вызывает `send()` (спит на IO), не triggеrит push-rotation. Если pull-flush его try_lock'ает успешно — drain'ится спокойно. Но если writer внутри активного `send()` и pull всегда попадает в try_lock-промах — данные задерживаются. | На полностью idle writer'е try_lock проходит без contention'а → pull-flush ротирует буфер штатно. В race-сценарии (writer внутри send) данные доедут либо через свой push-rotation на полном буфере, либо на следующем pull-проходе, либо в худшем случае через graceful shutdown'ный блокирующий drain. Bounded-latency for-pure-idle-writers удовлетворена, для активно пишущего — нет, но активный writer и так ротирует свой буфер быстро через push-путь. |
